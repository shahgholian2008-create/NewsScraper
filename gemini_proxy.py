...
def translate_to_persian(text, max_retries=3):
    """ترجمه با Groq از طریق Render Proxy، با قابلیت تلاش مجدد در صورت خطا"""
...
{text[:2000]}"""
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    print(f"   ⚠️ ساختار پاسخ غیرمنتظره: {result}")
                    return ""
            
            # اگه خطای 503 (سرویس در دسترس نیست) یا 429 (محدودیت نرخ) بود، دوباره تلاش کن
            elif response.status_code in [503, 429]:
                wait_time = (attempt + 1) * 5  # ۵، ۱۰، ۱۵ ثانیه صبر کن
                print(f"   ⏳ خطای {response.status_code}، {wait_time} ثانیه صبر و تلاش مجدد...")
                time.sleep(wait_time)
                continue
            
            else:
                print(f"   ❌ خطای {response.status_code}: {response.text[:200]}")
                return ""
                
        except requests.exceptions.Timeout:
            print(f"   ⏳ Timeout، تلاش مجدد...")
            time.sleep(5)
            continue
    ...
    print(f"   ❌ بعد از {max_retries} تلاش، ترجمه ناموفق بود.")
    return ""
